...
print("Libraries installed successfully!","hey hey hey.... boiiiiii")
import os
...
st.title("A Job Hunt practice")
st.markdown("# Cover Letter Generator...")
st.sidebar.markdown("# Cover Letter Generator->")

############# Below code will only add selectbox in one possible sidebar#######
# with st.sidebar: 
#     st.selectbox(
#      'testBar',
#      ('letsee', 'lessee'))
...
with st.sidebar: 
    ...
    description_permission = st.selectbox(
     'Description',
     ('Show', 'Do not show'))

    if description_permission == 'Show':
        if model_used == 'DaVinci': 
            ...
            """)
            ##### below is a syntax testing of more markdown cases inside an if condition
            # st.markdown("""
            # Good at: 
            #     - Complex intent
            #     - cause and effects
        ...
        # max_tokens = st.text_input("No description for you,", "You turned me off")
        max_tokens = st.text_input("Maximum number of tokens:", "1949")
